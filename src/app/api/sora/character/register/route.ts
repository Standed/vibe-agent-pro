...
export async function POST(req: Request) {
    try {
        ...
        const { characterId, projectId, mode } = body;
        ...
        if (characterId) {
            ...
            if (data) {
                character = {
                    ...
                    soraReferenceVideoUrl: data.metadata?.soraIdentity?.referenceVideoUrl || data.sora_reference_video_url, // try both based on schema
        ...
        if (mode === 'register_direct') {
            ...
            const result = await characterConsistencyService.registerCharacter(character, character.soraReferenceVideoUrl, userId);
