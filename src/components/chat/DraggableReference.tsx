...
export const DraggableReference: React.FC<DraggableReferenceProps> = ({ refItem, index, moveReference, onRemove }) => {
    ...
    return (
        ...
        >
            <img
                src={refItem.url}
                alt={refItem.label}
                className={`w-full h-full object-cover rounded-lg border ${refItem.source === 'manual_upload' ? 'border-green-500/50' : 'border-white/10'}`}
            />
            {/* 序号标记 - 可选 */}
