...
- 自动轮询进行中任务
- **智能排序**：优先按 `updatedAt` 排序，确保最新完成的任务排在最前
- **安全自动同步**：
  - 完成后自动同步到分镜（含 generationHistory）
  - **防覆盖机制**：引入 `mountTimeRef`，仅当任务是在当前会话中新完成的（或分镜当前无视频）才自动覆盖，防止页面刷新后旧任务覆盖用户手动选择的视频。
...
| v2.8 | 2026-01-19 | **简化纯文本提示词格式**：角色设定前置、场景信息、中文景别、台词末尾、AI自动分镜防重复触发 |
