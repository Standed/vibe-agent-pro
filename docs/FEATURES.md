...
#### 10. Pro 模式双向拖拽 (Bidirectional Drag-Drop) (v3.8)
- **Pro -> Storyboard**：支持将生成的 Grid 切片或单图直接拖拽到分镜卡片，自动保存到分镜历史。
- **Storyboard -> Pro**：支持将分镜列表中的缩略图拖拽到 Pro 聊天框，自动识别为参考图 (`shot_ref`)。
- **全屏预览拖拽**：支持从图片预览 Modal 中直接发起拖拽操作。

#### 11. 聊天面板重构 (ChatPanel Refactoring) (v3.8)
- **Hooks 架构**：核心逻辑拆分为 `useChatHistory` 和 `useAutoReference`。
- **性能优化**：大幅减少不必要的重渲染。
- **逻辑解耦**：UI 与业务逻辑分离，提升可维护性。
